...
class Punish:
    ...
    async def punish(self, ctx, user: discord.Member, duration: str=None, *, reason: str=None):
        ...
        await self.bot.add_roles(user, role)
        await self.bot.server_voice_state(user, mute=True, deafen=True)
        dataIO.save_json(self.location, self.json)
    ...
    async def warn(self, ctx, user: discord.Member, *, reason: str=None):
        ...
        msg.append("you is being a bitch "
                   "ass nigga.")
        if reason:
            msg.append("Holy shit, %s." % reason)
        msg.append("Fuck you and everything you stand for.")
        await self.bot.send_message(ctx.message.channel, ' '.join(msg), tts=False)

    @commands.command(pass_context=True, no_pm=True)
    @checks.mod_or_permissions(manage_messages=True)
    async def swarn(self, ctx, user: discord.Member, *, reason: str=None):
        """Warns a user with boilerplate about the rules."""
        msg = ['Hey %s, ' % user.mention]
        msg.append("you is being a bitch "
                   "ass nigga.")
        if reason:
            msg.append("Holy shit, %s." % reason)
        msg.append("Fuck you and everything you stand for.")
        await self.bot.send_message(ctx.message.channel, ' '.join(msg), tts=True)
    ...
    @checks.mod_or_permissions(manage_messages=True)
    async def storytime(self, ctx, user: discord.Member, *, duration: str=None):
        """Mutes everyone in the channel of the user besides themself."""
        if ctx.invoked_subcommand is None:
            await ctx.invoke(self.storytime_start, user=user, duration=duration)
    
    @checks.mod_or_permissions(manage_messages=True)
    @storytime.command(name="start", pass_context=True, no_pm=True)
    async def storytime_start(self, ctx, user: discord.Member, *, duration: str=None):
        ...
        if not user.voice_channel:
            ...
            users = user.voice_channel.voice_members
        x = []
        for member in users:
            if member != user: 
                x.append(member)
    ...
    @checks.mod_or_permissions(manage_messages=True)
    @storytime.command(name="end", pass_context=True, no_pm=True)    
    async def storytime_end(self, ctx, user: discord.Member = None):
        server = ctx.message.server
        if not user:
